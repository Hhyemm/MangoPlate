...
class AllViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        dataService.fetchGetTodayReviewData(self)
        dataService.fetchGetNewsData(self, "1,3,5")
    }
    ...
    func setSelect(_ tag: Int) {
        ...
        let x = selects.filter{$0.value}.map{String($0.key)}.joined(separator: ",")
        dataService.fetchGetNewsData(self, x)
    }
...
extension AllViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.item == 0 {
            ...
            (todayReveiw?.profileImgUrl) == nil ? cell.userImage.image = UIImage(named: "userBasicImage") : cell.userImage.load(url: URL(string: (todayReveiw?.profileImgUrl ?? ""))!)
            ...
            if todayReveiw?.imgUrls ?? [] != [] {
                cell.image.load(url: URL(string: (todayReveiw?.imgUrls?[0] ?? ""))!)
            }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "NewsCell", for: indexPath) as? NewsCell else {
                return .zero }
        return CGSize(width: collectionView.frame.width, height: cell.content.frame.height + cell.image.frame.height + 310)
    }
}

extension AllViewController {
    func didSuccessTodayReviewGetData(_ results: TodayReviewResult) {
    ...
    func didSuccessNewsGetData(_ results: [NewsResult]) {
        DispatchQueue.main.async {
